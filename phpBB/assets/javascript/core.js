...
/**
 * Update the next token according to raw token and session id
 */
function update_token() {
    ...
    let cookie_name = localStorage.getItem("cookie_name");
    let sid = localStorage.getItem("sid");
    const sid_matches = new RegExp("(phpbb3_[0-9a-zA-Z]+)_sid=([0-9a-zA-Z]+)").exec(document.cookie);
    if (sid_matches !== null) {
        cookie_name = sid_matches[1];
        sid = sid_matches[2];
        localStorage.setItem("cookie_name", cookie_name);
        localStorage.setItem("sid", sid);
    } else if (sid === null) {
        console.log("No sid found, user not logged in");
    }

    if (sid !== null) {
        const raw_token_matches = new RegExp(cookie_name + "_raw_token=([0-9a-zA-Z]+)").exec(document.cookie);
        if (raw_token_matches !== null) {
            const raw_token = raw_token_matches[1];
            invalidate_cookie(cookie_name + "_sid");
            invalidate_cookie(cookie_name + "_raw_token");
            document.cookie = cookie_name + "_token=" + md5(sid + raw_token);
        } else {
            const token_matches = new RegExp(cookie_name + "_token=([0-9a-zA-Z]+)").exec(document.cookie);
